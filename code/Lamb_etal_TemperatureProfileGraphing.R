...
combined_plot <- ggplot(Temp, aes(x = Day)) +
  geom_line(aes(y = ElevatedTemperature), color = "black", size = 1.2, linetype="dotdash") +
  geom_line(aes(y = RampTemp), color = "blue", size = 1.5, linetype="solid") +
  geom_line(aes(y = (DHW*0.308) + 27), color = "red", size = 1.2, linetype="dashed") +  # Multiply DHW by 5 to demonstrate the secondary axis
  ...
  geom_hline(yintercept=27.8, linetype="dotted", color = "black", size = 1.2)+
   xlab("Day") +
  theme_classic() +
